1. Sorted Merge: You are given two sorted arrays, A and B, where A has a large
enough buffer at the end to hold B.  Write a method to merge B in to A in
sorted order.

2. Group Anagrams: Write a method to sort an array of strings so that all
anagrams are next to eachother.

3. Search in Rotated Array: Given a sorted array of n integers that has been
...
4. Sorted Search, No Size: You are given an array-like data structure Listy
...
only supports positive integersss.)  Given a Listy which contains sorted,
positive integers, find the index at which an element x occurs.  If x occurs
multiple times, you may return any index.

5. Sparse Search: Given a sorted array of strings that is interspersed with
empty strings, write a method to find the location of a given string.
EXAMPLE
Input: ball, {"at", "", "", "", "", "ball", "", "", "car", "", "", "dad", "", ""}
Output: 4

6. Sort Big File: Imagine you have a 20GB file with one string per line.
Explain how you would sort the file.

7. Missing Int: Given an input file with four billion non-negative integers,
...
FOLLOW UP
What if you only have 10MB of memory?  Assume that all values are distinct
and we now have no more than one billion non-negative integers.

8. Find Duplicates: You have an array with all the numbers from 1 to N,
...
9. Sorted Matrix Search: Given an MxN matrix in which each row and each
column is sorted in ascending order, write a method to find an element.

10. Rank from Stream: Imagine you are reading in a stream of integers.
...
algorithms to support these operations.  That is, implement a method
...
11. Peaks and Valleys: In an array of integersm a "peak" is an element which
